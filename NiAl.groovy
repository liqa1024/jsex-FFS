...
import static jsex.rareevent.lmp.NativeLmpFullPathGenerator.LANGEVIN

final def workingDirIn = args ? args[0] : 'NiAl-FFS'
...
final int FFSTemp           = 1300
final int initRunStep       = 500000
final int FFSRunStep        = 5000000
...
final int dumpStep          = 500 // tau_gap = 1 ps for timestep == 0.02
...
final int pruningStep       = MathEX.Code.divup(50000, dumpStep) // 100 ps for timestep == 0.02
...
MultipleNativeLmpFullPathGenerator.withOf(subComm, subRoots, dumpCal, initPoints, [MASS.Ni, MASS.Al], FFSTemp, pairStyle, pairCoeff, timestep, dumpStep) {fullPathGen ->
    ...
    /** begin FFS */
    println("=====BEGIN FFS OF Ni${Ni}Al${Al}=====")
    ...
    try (def FFS = new ForwardFluxSampling<>(fullPathGen, parallelNum, surfaceA, surfaces, N0).setStep1Mul(step1Mul).setPruningProb(pruningProb).setPruningThreshold(pruningThreshold).setPruningStep(pruningStep).setPruningPolicy(pruningPolicy).setMaxPathNum(N0 * 10000)) {
        ...
        while (!FFS.finished()) {
            ...
            UT.Timer.tic()
            FFS.run()
            UT.Timer.toc("i = ${FFS.step()-1}, prob = ${FFS.getProb(FFS.step()-1)}, step2PointNum = ${FFS.step2PointNum(FFS.step()-1)}, step2PathNum = ${FFS.step2PathNum(FFS.step()-1)},")
            if (printPathEff) {
                def info = fullPathGen.getTimerInfo()
                println("PathGenEff: lmp = ${percent(info.lmp/info.total)}, lambda = ${percent(info.lambda/info.total)}, wait = ${percent(info.wait/info.total)}, else = ${percent(info.other/info.total)}")
            }
            ...
            if (dumpAllPath) {
                for (j in range(FFS.pointsOnLambda().size())) Dump.fromAtomDataList(FFS.pickPath(j)).write(FFSAllDumpPath + '/' + j)
                UT.IO.dir2zip(FFSAllDumpPath, "${FFSAllDumpPath}-${FFS.step()}.zip")
                UT.IO.rmdir(FFSAllDumpPath)
            }
            if (FFS.stepFinished()) {
                Dump.fromAtomDataList(FFS.pointsOnLambda()).write(FFSRestartPathDu)
